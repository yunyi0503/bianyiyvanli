...
void block_syntax::print()
{
    ...
    if (this->body.empty()) {
        // 如果block为空，不打印内容
        ast_printer.LevelPrint(std::cout,"<empty block>",true);
    } else {
        for(auto & content: this->body){
            if (content) { // 检查指针有效性
                content.get()->print();
            } else {
                ast_printer.LevelPrint(std::cout,"<null statement>",true);
            }
        }
    }
    ast_printer.cur_level--;
...
void if_stmt_syntax::print()
{
    ...
    if (this->pred) {
        ...
        ast_printer.LevelPrint(std::cout,"<null condition>",true);
    }
